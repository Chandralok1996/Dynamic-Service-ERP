...
import { AdminService, AppService } from "src/app/_services";
  ...
  getIncidentList() {
    ...
    this.adminService.incidentList(this.status1).subscribe((res: any) => {
      ...
      if (this.status == "Hardware") {
        this.reportStatusData = this.reportData.filter(
          (ele: any) => ele.Category == "Hardware"
        );
      } else if (this.status == "Software") {
        this.reportStatusData = this.reportData.filter(
          (ele: any) => ele.Category == "Software"
        );
      } else if (this.status == "Incident") {
        this.reportStatusData = this.reportData.filter(
          (ele: any) => ele["Call Type"] == "Incident"
        );
      } else if (this.status == "Request") {
        this.reportStatusData = this.reportData.filter(
          (ele: any) => ele["Call Type"] == "Request"
        );
      } else if (this.status == "Web") {
        this.reportStatusData = this.reportData.filter(
          (ele: any) => ele["Call Mode"] == "Web"
        );
      } else if (this.status == "Mobile App") {
        this.reportStatusData = this.reportData.filter(
          (ele: any) => ele["Call Mode"] == "Mobile App"
        );
      }
