...
export class ItemCreateComponent {
  ...
  itemCreated: any = localStorage.getItem('user-created');
  ...
  constructor(private toaster: ToasterService, private adminService: AdminService, private router: Router, private formBuilder: FormBuilder) {
    ...
    if(!this.itemCreated) {
      ...
      this.itemCreated = JSON.parse(this.itemCreated);
    }
  }
  ...
  submitForm() {
    ...
    this.itemCreated.push(match);
console.log(match);
    this.adminService.itemCreate(match).subscribe((res:any)=>{
    })
    localStorage.setItem('user-created', JSON.stringify(this.itemCreated));
